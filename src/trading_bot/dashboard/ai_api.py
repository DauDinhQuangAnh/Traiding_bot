"""GET/HEAD-only local API for persisted AI benchmark evidence."""
...
class ReadOnlyAIBenchmarkApi:
    _SECTIONS: ClassVar = {"metrics", "cases", "responses"}

    def __init__(self, service: AIBenchmarkDashboardService) -> None:
    ...
    def _read(self, path: str, *, offset: int, limit: int) -> ApiResponse:
        ...
        if parts == ("ai-benchmark-runs",):
            return ApiResponse(200, self._service.list_runs(offset=offset, limit=limit))
        if len(parts) == 2 and parts[0] == "ai-benchmark-runs":
            return ApiResponse(200, self._service.get_run(parts[1]))
        if len(parts) == 3 and parts[0] == "ai-benchmark-runs" and parts[2] in self._SECTIONS:
            return ApiResponse(200, self._service.get_section(parts[1], parts[2]))
        raise PersistenceError("unknown AI dashboard route")
